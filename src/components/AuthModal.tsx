...
export default function AuthModal({ onSuccess, onClose }: AuthModalProps) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      const profile = await syncAndLinkUserProfile(cred.user, {
        name: cred.user.displayName || "Homeowner",
        email: cred.user.email || "",
        role: "resident",
        address: "Oakridge Estates Community",
        phone: cred.user.phoneNumber || "(555) 000-0000"
      });
      setLoading(false);
      onSuccess(profile);
    } catch (err: any) {
          ...
          {!isForgot && (
            <div className="space-y-3">
              <button
                ...
                className="w-full bg-white hover:bg-slate-50 active:bg-slate-100 text-slate-800 font-bold py-2.5 px-4 rounded-xl border border-slate-200 shadow-2xs flex items-center justify-center gap-3 transition-all text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50"
                id="btn-google-auth"
              >
